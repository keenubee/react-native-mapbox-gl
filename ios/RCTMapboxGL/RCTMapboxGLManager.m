...
- (NSDictionary*)geoJSONGeometryFromMGLFeature:(id <MGLFeature>)feature
{
    ...
    // } else if ([feature isKindOfClass:[MGLPointCollectionFeature class]]) {
    //     // this is checked last since MGLPolyline and MGLPolygon inherit from MGLMultiPoint
    //     geometryType = @"PointCollection";
    //     MGLPointCollectionFeature *pointCollection = (MGLPointCollectionFeature *)feature;
    //     for (int index = 0; index < pointCollection.pointCount; index++) {
    //         CLLocationCoordinate2D coord = pointCollection.coordinates[index];
